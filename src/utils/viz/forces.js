...
export const collide = sim =>
  sim.force('collide', d3.forceCollide(25).strength(0.3));
...
export const boardForce = game => (sim) => {
  const a = 0;
  const dist = link => Math.hypot(...boardDelta(link));

  return sim.force('board',
    d3.forceLink(boardLinks(game)).id(d => d.id)
...
export const playerForce = game => (sim) => {
  ...
  return sim.force('players',
    d3.forceLink(playerLinks(game)).id(d => d.id)
